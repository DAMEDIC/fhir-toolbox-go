...
func (r Practitioner) MemSize() int {
	...
	s += (cap(r.Qualification) - len(r.Qualification)) * int(reflect.TypeOf(PractitionerQualification{}).Size())
	for _, i := range r.Communication {
		s += i.MemSize()
	}
	s += (cap(r.Communication) - len(r.Communication)) * int(reflect.TypeOf(PractitionerCommunication{}).Size())
	return s
}
...
func (r PractitionerCommunication) String() string {
	buf, err := json.MarshalIndent(r, "", "  ")
	if err != nil {
		return "null"
	}
...
func (r PractitionerCommunication) Equivalent(other fhirpath.Element, _noReverseTypeConversion ...bool) bool {
	o, ok := other.(PractitionerCommunication)
	if !ok {
		return false
	}
	r.Id = nil
