...
func (r ChargeItem) MemSize() int {
	...
	s += (cap(r.Service) - len(r.Service)) * int(reflect.TypeOf(CodeableReference{}).Size())
	for _, i := range r.Product {
		s += i.MemSize()
	}
	s += (cap(r.Product) - len(r.Product)) * int(reflect.TypeOf(CodeableReference{}).Size())
...
func (r ChargeItem) marshalJSON(w io.Writer) error {
	...
	if r.UnitPriceComponent != nil {
		...
		setComma = true
		_, err = w.Write([]byte("\"unitPriceComponent\":"))
		if err != nil {
			return err
		}
		err = r.UnitPriceComponent.marshalJSON(w)
		if err != nil {
			return err
		}
	}
	if r.TotalPriceComponent != nil {
		...
		err = r.TotalPriceComponent.marshalJSON(w)
		if err != nil {
			return err
		}
	}
	if r.OverrideReason != nil {
		...
		err = r.OverrideReason.marshalJSON(w)
		if err != nil {
			return err
		}
	}
...
func (r ChargeItem) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.UnitPriceComponent, xml.StartElement{Name: xml.Name{Local: "unitPriceComponent"}})
	if err != nil {
		return err
	}
...
func (r ChargeItem) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "unitPriceComponent") {
		if r.UnitPriceComponent != nil {
			children = append(children, *r.UnitPriceComponent)
		}
	}
	if len(name) == 0 || slices.Contains(name, "totalPriceComponent") {
		if r.TotalPriceComponent != nil {
			children = append(children, *r.TotalPriceComponent)
		}
	}
