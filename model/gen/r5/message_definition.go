...
func (r MessageDefinition) marshalJSON(w io.Writer) error {
	...
	if r.Graph != nil && r.Graph.Value != nil {
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
	...
	if r.Graph != nil && (r.Graph.Id != nil || r.Graph.Extension != nil) {
		p := primitiveElement{Id: r.Graph.Id, Extension: r.Graph.Extension}
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
...
func (r MessageDefinition) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "graph") {
		if r.Graph != nil {
			children = append(children, *r.Graph)
		}
	}
	return children
}
