...
func (r Distance) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
...
func (r *Distance) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in Distance element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in Distance element", t)
		}
		switch f {
		case "id":
			var v string
		...
		default:
			return fmt.Errorf("invalid field: %s in Distance", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r Distance) Children(name ...string) fhirpath.Collection {
