...
func (r String) isOperationDefinitionVersionAlgorithm() {}
func (r Coding) isOperationDefinitionVersionAlgorithm() {}
...
func (r OperationDefinitionParameter) MemSize() int {
	...
	s += (cap(r.Scope) - len(r.Scope)) * int(reflect.TypeOf(Code{}).Size())
	...
	s += (cap(r.AllowedType) - len(r.AllowedType)) * int(reflect.TypeOf(Code{}).Size())
...
func (r OperationDefinitionParameter) marshalJSON(w io.Writer) error {
	...
	}
	{
		...
		if anyValue {
			...
			setComma = true
			_, err = w.Write([]byte("\"allowedType\":"))
			if err != nil {
				return err
			}
			enc := json.NewEncoder(w)
			enc.SetEscapeHTML(false)
			err := enc.Encode(r.AllowedType)
			if err != nil {
				return err
			}
		}
		...
		if anyIdOrExtension {
			...
			setComma = true
			_, err = w.Write([]byte("\"_allowedType\":"))
			if err != nil {
				return err
			}
			_, err = w.Write([]byte("["))
	...
	for d.More() {
		...
		case "scope":
			...
			for i := 0; d.More(); i++ {
				...
				for len(r.Scope) <= i {
					r.Scope = append(r.Scope, Code{})
				}
				r.Scope[i].Value = v.Value
			}
			t, err = d.Token()
			if err != nil {
				return err
		...
		case "_scope":
			...
			for i := 0; d.More(); i++ {
				...
				r.Scope[i].Id = v.Id
				r.Scope[i].Extension = v.Extension
			}
			t, err = d.Token()
			if err != nil {
				return err
		...
		case "allowedType":
			...
			for i := 0; d.More(); i++ {
				...
				for len(r.AllowedType) <= i {
					r.AllowedType = append(r.AllowedType, Code{})
				}
				r.AllowedType[i].Value = v.Value
			}
			t, err = d.Token()
			if err != nil {
				return err
		...
		case "_allowedType":
			...
			for i := 0; d.More(); i++ {
				...
				r.AllowedType[i].Id = v.Id
				r.AllowedType[i].Extension = v.Extension
			}
			t, err = d.Token()
			if err != nil {
				return err
...
func (r OperationDefinitionParameter) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.AllowedType, xml.StartElement{Name: xml.Name{Local: "allowedType"}})
	if err != nil {
		return err
	}
...
func (r OperationDefinitionParameter) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "allowedType") {
		for _, v := range r.AllowedType {
			children = append(children, v)
		}
	}
