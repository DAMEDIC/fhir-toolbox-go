...
func (r Group) MemSize() int {
	...
	s += r.Membership.MemSize() - int(reflect.TypeOf(r.Membership).Size())
	if r.Code != nil {
		s += r.Code.MemSize()
	}
	if r.Name != nil {
...
func (r Group) marshalJSON(w io.Writer) error {
	...
	}
	{
		...
		setComma = true
		_, err = w.Write([]byte("\"membership\":"))
		if err != nil {
			return err
		}
		enc := json.NewEncoder(w)
		enc.SetEscapeHTML(false)
		err := enc.Encode(r.Membership)
		if err != nil {
			return err
		}
	}
	if r.Membership.Id != nil || r.Membership.Extension != nil {
		p := primitiveElement{Id: r.Membership.Id, Extension: r.Membership.Extension}
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
		...
		setComma = true
		_, err = w.Write([]byte("\"_membership\":"))
		if err != nil {
			return err
		}
		err = p.marshalJSON(w)
...
func (r *Group) unmarshalJSON(d *json.Decoder) error {
	...
	for d.More() {
		...
		case "_membership":
			...
			r.Membership.Id = v.Id
			r.Membership.Extension = v.Extension
		case "code":
...
func (r Group) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.Membership, xml.StartElement{Name: xml.Name{Local: "membership"}})
	if err != nil {
		return err
	}
