...
func (r Boolean) isPersonDeceased()  {}
func (r DateTime) isPersonDeceased() {}
...
func (r PersonCommunication) String() string {
	buf, err := json.MarshalIndent(r, "", "  ")
	if err != nil {
		return "null"
	}
...
func (r PersonCommunication) MarshalJSON() ([]byte, error) {
	var b bytes.Buffer
	err := r.marshalJSON(&b)
	if err != nil {
		return nil, err
...
func (r PersonCommunication) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
...
func (r *PersonCommunication) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in PersonCommunication element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in PersonCommunication element", t)
		}
		switch f {
		case "id":
			var v string
		...
		default:
			return fmt.Errorf("invalid field: %s in PersonCommunication", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r PersonCommunication) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if r.Id != nil {
...
func (r Person) Children(name ...string) fhirpath.Collection {
...
func (r PersonCommunication) Equivalent(other fhirpath.Element, _noReverseTypeConversion ...bool) bool {
	o, ok := other.(PersonCommunication)
	if !ok {
		return false
	}
	r.Id = nil
...
func (r PersonLink) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "assurance") {
		if r.Assurance != nil {
			children = append(children, *r.Assurance)
		}
	}
	return children
}
