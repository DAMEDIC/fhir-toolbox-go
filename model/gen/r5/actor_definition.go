...
func (r String) isActorDefinitionVersionAlgorithm() {}
func (r Coding) isActorDefinitionVersionAlgorithm() {}
func (r ActorDefinition) ResourceType() string {
	return "ActorDefinition"
}
func (r ActorDefinition) ResourceId() (string, bool) {
	if r.Id == nil {
		return "", false
	}
	if r.Id.Value == nil {
...
func (r ActorDefinition) MemSize() int {
	var emptyIface any
...
func (r ActorDefinition) String() string {
...
func (r ActorDefinition) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
	}
	_, err = w.Write([]byte("\"resourceType\":\"ActorDefinition\""))
	if err != nil {
		return err
	}
	setComma := true
	...
	if r.Capabilities != nil && r.Capabilities.Value != nil {
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
	...
	if r.Capabilities != nil && (r.Capabilities.Id != nil || r.Capabilities.Extension != nil) {
...
func (r *ActorDefinition) UnmarshalJSON(b []byte) error {
	d := json.NewDecoder(bytes.NewReader(b))
	return r.unmarshalJSON(d)
}
func (r *ActorDefinition) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in ActorDefinition element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in ActorDefinition element", t)
		}
		switch f {
		case "resourceType":
			_, err := d.Token()
		...
		default:
			return fmt.Errorf("invalid field: %s in ActorDefinition", f)
		}
	}
	t, err = d.Token()
	if err != nil {
