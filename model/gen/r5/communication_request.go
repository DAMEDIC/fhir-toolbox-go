...
func (r CodeableConcept) isCommunicationRequestPayloadContent() {}
...
func (r CommunicationRequest) marshalJSON(w io.Writer) error {
	...
	if len(r.InformationProvider) > 0 {
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
		...
		setComma = true
		_, err = w.Write([]byte("\"informationProvider\":"))
		if err != nil {
			return err
		}
		_, err = w.Write([]byte("["))
		...
		setComma = false
		for _, e := range r.InformationProvider {
			if setComma {
				_, err = w.Write([]byte(","))
				if err != nil {
					return err
...
func (r CommunicationRequest) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.InformationProvider, xml.StartElement{Name: xml.Name{Local: "informationProvider"}})
	if err != nil {
		return err
	}
