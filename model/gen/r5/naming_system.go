...
func (r String) isNamingSystemVersionAlgorithm() {}
func (r Coding) isNamingSystemVersionAlgorithm() {}
...
func (r NamingSystem) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
	}
	_, err = w.Write([]byte("\"resourceType\":\"NamingSystem\""))
	if err != nil {
		return err
	}
	setComma := true
...
func (r NamingSystemUniqueId) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
	...
	if r.Authoritative != nil && r.Authoritative.Value != nil {
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
		...
		setComma = true
		_, err = w.Write([]byte("\"authoritative\":"))
		if err != nil {
			return err
		}
		enc := json.NewEncoder(w)
		enc.SetEscapeHTML(false)
		err := enc.Encode(r.Authoritative)
		if err != nil {
			return err
		}
	}
	if r.Authoritative != nil && (r.Authoritative.Id != nil || r.Authoritative.Extension != nil) {
		p := primitiveElement{Id: r.Authoritative.Id, Extension: r.Authoritative.Extension}
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
		...
		setComma = true
		_, err = w.Write([]byte("\"_authoritative\":"))
		if err != nil {
			return err
		}
		err = p.marshalJSON(w)
...
func (r *NamingSystem) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in NamingSystem element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in NamingSystem element", t)
		}
		switch f {
		case "resourceType":
			_, err := d.Token()
		...
		default:
			return fmt.Errorf("invalid field: %s in NamingSystem", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r *NamingSystemUniqueId) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in NamingSystemUniqueId element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in NamingSystemUniqueId element", t)
		}
		switch f {
		case "id":
			var v string
		...
		case "authoritative":
			...
			if r.Authoritative == nil {
				r.Authoritative = &Boolean{}
			}
			r.Authoritative.Value = v.Value
		case "_authoritative":
			var v primitiveElement
			err := v.unmarshalJSON(d)
			if err != nil {
			...
			r.Authoritative.Id = v.Id
			r.Authoritative.Extension = v.Extension
		default:
			return fmt.Errorf("invalid field: %s in NamingSystemUniqueId", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r NamingSystemUniqueId) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.Authoritative, xml.StartElement{Name: xml.Name{Local: "authoritative"}})
	if err != nil {
		return err
	}
	err = e.EncodeToken(start.End())
...
func (r NamingSystem) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "uniqueId") {
		for _, v := range r.UniqueId {
			children = append(children, v)
		}
	}
	return children
...
func (r NamingSystemUniqueId) Children(name ...string) fhirpath.Collection {
	var children fhirpath.Collection
	if len(name) == 0 || slices.Contains(name, "id") {
		if r.Id != nil {
	...
	if len(name) == 0 || slices.Contains(name, "authoritative") {
		if r.Authoritative != nil {
			children = append(children, *r.Authoritative)
		}
	}
	return children
}
