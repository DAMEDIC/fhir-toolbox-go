...
func (r ActivityDefinition) MemSize() int {
	...
	s += (cap(r.SpecimenRequirement) - len(r.SpecimenRequirement)) * int(unsafe.Sizeof(Canonical{}))
	for _, i := range r.ObservationRequirement {
		s += i.MemSize()
	}
	s += (cap(r.ObservationRequirement) - len(r.ObservationRequirement)) * int(unsafe.Sizeof(Canonical{}))
	for _, i := range r.ObservationResultRequirement {
		s += i.MemSize()
	}
	s += (cap(r.ObservationResultRequirement) - len(r.ObservationResultRequirement)) * int(unsafe.Sizeof(Canonical{}))
	if r.Transform != nil {
