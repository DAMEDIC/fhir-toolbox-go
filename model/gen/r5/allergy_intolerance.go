...
func (r AllergyIntolerance) MemSize() int {
	...
	s += (cap(r.Participant) - len(r.Participant)) * int(reflect.TypeOf(AllergyIntoleranceParticipant{}).Size())
...
func (r AllergyIntoleranceParticipant) MemSize() int {
	s := int(reflect.TypeOf(r).Size())
	if r.Id != nil {
...
func (r AllergyIntoleranceReaction) MemSize() int {
	...
	s += (cap(r.Manifestation) - len(r.Manifestation)) * int(reflect.TypeOf(CodeableReference{}).Size())
...
func (r AllergyIntoleranceParticipant) MarshalJSON() ([]byte, error) {
	var b bytes.Buffer
	err := r.marshalJSON(&b)
	if err != nil {
		return nil, err
...
func (r AllergyIntoleranceParticipant) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
...
func (r *AllergyIntoleranceParticipant) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in AllergyIntoleranceParticipant element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in AllergyIntoleranceParticipant element", t)
		}
		switch f {
		case "id":
			var v string
		...
		case "actor":
			...
			r.Actor = v
		default:
			return fmt.Errorf("invalid field: %s in AllergyIntoleranceParticipant", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r AllergyIntoleranceParticipant) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if r.Id != nil {
...
func (r AllergyIntoleranceParticipant) Equivalent(other fhirpath.Element, _noReverseTypeConversion ...bool) bool {
	o, ok := other.(AllergyIntoleranceParticipant)
	if !ok {
		return false
	}
	r.Id = nil
