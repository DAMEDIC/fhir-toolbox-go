...
func (r Organization) MemSize() int {
	...
	s += (cap(r.Qualification) - len(r.Qualification)) * int(reflect.TypeOf(OrganizationQualification{}).Size())
	return s
}
func (r OrganizationQualification) MemSize() int {
	s := int(reflect.TypeOf(r).Size())
	if r.Id != nil {
...
func (r OrganizationQualification) String() string {
	buf, err := json.MarshalIndent(r, "", "  ")
	if err != nil {
		return "null"
	}
...
func (r OrganizationQualification) MarshalJSON() ([]byte, error) {
	var b bytes.Buffer
	err := r.marshalJSON(&b)
	if err != nil {
		return nil, err
...
func (r OrganizationQualification) marshalJSON(w io.Writer) error {
	var err error
	_, err = w.Write([]byte("{"))
	if err != nil {
		return err
...
func (r *OrganizationQualification) unmarshalJSON(d *json.Decoder) error {
	t, err := d.Token()
	if err != nil {
		return err
	}
	if t != json.Delim('{') {
		return fmt.Errorf("invalid token: %v, expected: '{' in OrganizationQualification element", t)
	}
	for d.More() {
		t, err = d.Token()
		if err != nil {
		...
		f, ok := t.(string)
		if !ok {
			return fmt.Errorf("invalid token: %v, expected: field name in OrganizationQualification element", t)
		}
		switch f {
		case "id":
			var v string
		...
		case "issuer":
			...
			r.Issuer = &v
		default:
			return fmt.Errorf("invalid field: %s in OrganizationQualification", f)
		}
	}
	t, err = d.Token()
	if err != nil {
...
func (r OrganizationQualification) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if r.Id != nil {
...
func (r OrganizationQualification) Equivalent(other fhirpath.Element, _noReverseTypeConversion ...bool) bool {
	o, ok := other.(OrganizationQualification)
	if !ok {
		return false
	}
	r.Id = nil
