...
func (r RiskEvidenceSynthesis) MemSize() int {
	...
	s += r.Population.MemSize() - int(unsafe.Sizeof(r.Population))
	if r.Exposure != nil {
	...
	s += (cap(r.Certainty) - len(r.Certainty)) * int(unsafe.Sizeof(RiskEvidenceSynthesisCertainty{}))
	return s
}
...
func (r RiskEvidenceSynthesisRiskEstimate) MemSize() int {
	...
	s += (cap(r.PrecisionEstimate) - len(r.PrecisionEstimate)) * int(unsafe.Sizeof(RiskEvidenceSynthesisRiskEstimatePrecisionEstimate{}))
	return s
}
...
func (r RiskEvidenceSynthesisCertainty) MemSize() int {
	...
	s += (cap(r.Rating) - len(r.Rating)) * int(unsafe.Sizeof(CodeableConcept{}))
	for _, i := range r.Note {
		s += i.MemSize()
	}
	s += (cap(r.Note) - len(r.Note)) * int(unsafe.Sizeof(Annotation{}))
	...
	s += (cap(r.CertaintySubcomponent) - len(r.CertaintySubcomponent)) * int(unsafe.Sizeof(RiskEvidenceSynthesisCertaintyCertaintySubcomponent{}))
	return s
}
