...
func (r Endpoint) MemSize() int {
	...
	s += r.ConnectionType.MemSize() - int(reflect.TypeOf(r.ConnectionType).Size())
	if r.Name != nil {
		s += r.Name.MemSize()
	}
	...
	s += (cap(r.PayloadType) - len(r.PayloadType)) * int(reflect.TypeOf(CodeableConcept{}).Size())
	for _, i := range r.PayloadMimeType {
		s += i.MemSize()
	}
	s += (cap(r.PayloadMimeType) - len(r.PayloadMimeType)) * int(reflect.TypeOf(Code{}).Size())
...
func (r Endpoint) marshalJSON(w io.Writer) error {
	...
	err = r.ConnectionType.marshalJSON(w)
	if err != nil {
		return err
	}
	...
	if len(r.PayloadType) > 0 {
		...
		setComma = true
		_, err = w.Write([]byte("\"payloadType\":"))
		if err != nil {
			return err
		}
		_, err = w.Write([]byte("["))
	...
	}
	{
		...
		if anyValue {
			...
			setComma = true
			_, err = w.Write([]byte("\"payloadMimeType\":"))
			if err != nil {
				return err
			}
			enc := json.NewEncoder(w)
			enc.SetEscapeHTML(false)
			err := enc.Encode(r.PayloadMimeType)
			if err != nil {
				return err
			}
		}
		...
		if anyIdOrExtension {
			...
			setComma = true
			_, err = w.Write([]byte("\"_payloadMimeType\":"))
			if err != nil {
				return err
			}
			_, err = w.Write([]byte("["))
	...
	for d.More() {
		...
		case "payloadMimeType":
			...
			for i := 0; d.More(); i++ {
				...
				for len(r.PayloadMimeType) <= i {
					r.PayloadMimeType = append(r.PayloadMimeType, Code{})
				}
				r.PayloadMimeType[i].Value = v.Value
			}
			t, err = d.Token()
			if err != nil {
				return err
		...
		case "_payloadMimeType":
			...
			for i := 0; d.More(); i++ {
				...
				r.PayloadMimeType[i].Id = v.Id
				r.PayloadMimeType[i].Extension = v.Extension
			}
			t, err = d.Token()
			if err != nil {
				return err
...
func (r Endpoint) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	err = e.EncodeElement(r.PayloadType, xml.StartElement{Name: xml.Name{Local: "payloadType"}})
	if err != nil {
		return err
	}
	err = e.EncodeElement(r.PayloadMimeType, xml.StartElement{Name: xml.Name{Local: "payloadMimeType"}})
	if err != nil {
		return err
	}
...
func (r Endpoint) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "connectionType") {
		children = append(children, r.ConnectionType)
	}
	...
	if len(name) == 0 || slices.Contains(name, "payloadType") {
		for _, v := range r.PayloadType {
			children = append(children, v)
		}
	}
	if len(name) == 0 || slices.Contains(name, "payloadMimeType") {
		for _, v := range r.PayloadMimeType {
			children = append(children, v)
		}
	}
