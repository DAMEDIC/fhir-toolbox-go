...
func (r Evidence) MemSize() int {
	...
	s += r.ExposureBackground.MemSize() - int(reflect.TypeOf(r.ExposureBackground).Size())
	for _, i := range r.ExposureVariant {
		s += i.MemSize()
	}
	s += (cap(r.ExposureVariant) - len(r.ExposureVariant)) * int(reflect.TypeOf(Reference{}).Size())
	for _, i := range r.Outcome {
		s += i.MemSize()
	}
	s += (cap(r.Outcome) - len(r.Outcome)) * int(reflect.TypeOf(Reference{}).Size())
	return s
}
...
func (r Evidence) marshalJSON(w io.Writer) error {
	...
	setComma = true
	_, err = w.Write([]byte("\"exposureBackground\":"))
	if err != nil {
		return err
	}
	err = r.ExposureBackground.marshalJSON(w)
	if err != nil {
		return err
	}
	if len(r.ExposureVariant) > 0 {
		if setComma {
			_, err = w.Write([]byte(","))
			if err != nil {
				return err
		...
		setComma = true
		_, err = w.Write([]byte("\"exposureVariant\":"))
		if err != nil {
			return err
		}
		_, err = w.Write([]byte("["))
		...
		setComma = false
		for _, e := range r.ExposureVariant {
			if setComma {
				_, err = w.Write([]byte(","))
				if err != nil {
					return err
...
func (r Evidence) Children(name ...string) fhirpath.Collection {
	...
	if len(name) == 0 || slices.Contains(name, "exposureBackground") {
		children = append(children, r.ExposureBackground)
	}
	if len(name) == 0 || slices.Contains(name, "exposureVariant") {
		for _, v := range r.ExposureVariant {
			children = append(children, v)
		}
	}
