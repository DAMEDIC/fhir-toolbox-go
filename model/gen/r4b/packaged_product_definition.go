...
func (r PackagedProductDefinitionPackage) MemSize() int {
	...
	s += (cap(r.ShelfLifeStorage) - len(r.ShelfLifeStorage)) * int(reflect.TypeOf(PackagedProductDefinitionPackageShelfLifeStorage{}).Size())
	for _, i := range r.Manufacturer {
		s += i.MemSize()
	}
	s += (cap(r.Manufacturer) - len(r.Manufacturer)) * int(reflect.TypeOf(Reference{}).Size())
	...
	s += (cap(r.Property) - len(r.Property)) * int(reflect.TypeOf(PackagedProductDefinitionPackageProperty{}).Size())
	for _, i := range r.ContainedItem {
		s += i.MemSize()
	}
	s += (cap(r.ContainedItem) - len(r.ContainedItem)) * int(reflect.TypeOf(PackagedProductDefinitionPackageContainedItem{}).Size())
	for _, i := range r.Package {
		s += i.MemSize()
	}
	s += (cap(r.Package) - len(r.Package)) * int(reflect.TypeOf(PackagedProductDefinitionPackage{}).Size())
	return s
}
